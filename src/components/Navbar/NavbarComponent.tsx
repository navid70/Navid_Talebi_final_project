...
export default function NavbarComponent() {
    ...
    return (
        <>
            <Container className="navbar-dress py-4 d-flex align-items-center justify-content-between flex-row-reverse">
                <div className="order-lg-1 brand ">
                    ...
                    >
                        ...
                        <img className="img-brand" src={logo} alt=""/><h2 className='logoBrand pt-4'>LP shop</h2>
                    </Link>
                </div>
                ...
                <div className="d-none d-lg-flex  align-items-center">
                    <div>
                        ...
                        >
                            ...
                            <BiShoppingBag color="white" size={35}/>
                        </Button>

                        <div className="cart__modal py-4 text-white">
                            <Container>
                                    ...
                                    cart.map((item) => (
                                        ...
                                        <div key={item.id} className="cart__item">
                                            <Row>
                                                ...
                                                <Col sm={8}>
                                                    <div
                                                        className="desc__cart h-100 d-flex flex-column justify-content-evenly ">
                                                            ...
                                                            {item.discount ? (
                                                                <>
                                                                    <strong>
                                                                        {item.discount &&
                                                                        (item.price -
                                                                        (item.price * item.discount) / 100).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" "}
                                                                        تومان
                                                                    </strong>
                                                                    <small
                                                                        ...
                                                                        style={{color: "#f37a23"}}
                                                                    >
                                                                        {item.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} تومان
                                                                    </small>
                                                                </>
                                                            ) : (
                                                                <small
                                                                    ...
                                                                    style={{color: "#f37a23"}}
                                                                >
                                                                    {(item.price / 100).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} تومان
                                                                </small>
                                                            )}
                                                        </div>
                                                    </div>
                    ...
                    <Row className="p-3">
                        {
                            itemsSearch.map((item: IProduct) => (
                                <Col key={item.id} lg={3} className="d-flex flex-column mb-2">
                                    <small onClick={() => history.push(`/category/${item.category}/${item.id}`)}
                                           className="mb-2 product-search"> <small
                                        className="search-text">  {searchNav} در</small> {item.type}</small>
                                </Col>
                            ))
                        }
