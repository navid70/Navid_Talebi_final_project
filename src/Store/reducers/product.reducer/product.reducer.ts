...
const productReducer = createSlice({
    ...
    reducers: {
        productChangeClickImg: (state, action: PayloadAction<number>) => {
            state.productsCategory = state.productsCategory.map((item: IProduct, index: number) => item.id === action.payload ? {
                ...item,
                img: item.subImg
            } : item)
        },
        productChangeClickSubImg: (state, action: PayloadAction<{ id: number, category: string }>) => {
            state.productsCategory = state.productsData.filter((item) => item.category === action.payload.category).map((item: IProduct, index: number) => item.id === action.payload.id ? {
                ...item,
                img: item.img
            } : item)
        },
